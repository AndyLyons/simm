...
function createObjectWithPath(path, value) {
  return path.reduceRight(function (nextData, key) {
...
/**
 * Returns a unique string for different primitive types. The typeof operator
 * can return the same value for many different types e.g.
 *
 * | Type      | typeof      | Object.prototype.toString.apply |
 * |:----------|:------------|:--------------------------------|
...
function set(processors, prev, path, next) {
	...
	var processor = processors.find(prev);
	var key = path[0];
	var updatedChild = void 0;

	if (processor.has(prev, key)) {
		var child = processor.get(prev, key);
